...
class ForwardNavierStokes2d(BaseOperator):
    ...
    @torch.no_grad()
    def __call__(self, x, unnormalize=True):
        '''
        Args:
    ...
    def forward(self, x, unnormalize=True):
        ...
        if torch.isnan(sol).any():
            raise ValueError("NaN detected in the solution.")
    ...
    def gradient(self, pred, observation):
        raise NotImplementedError("Navier-Stokes2d problem does not allow for gradient access.")
